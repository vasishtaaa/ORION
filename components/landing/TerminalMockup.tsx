...
import { Activity, ArrowRight, ShieldCheck, Zap, TrendingUp, DollarSign } from 'lucide-react';
...
export function TerminalMockup() {
  return (
    <div className="relative w-full rounded-2xl sm:rounded-3xl p-1 bg-gradient-to-b from-white/15 via-white/5 to-transparent shadow-[0_20px_80px_rgba(0,0,0,0.9)] overflow-hidden group">
      ...
      {/* Frame Container */}
      <div className="relative z-10 w-full rounded-[14px] sm:rounded-[22px] bg-[#0b0f17] border border-white/10 p-4 sm:p-6 flex flex-col gap-5 overflow-hidden">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-white/10 flex-wrap gap-3">
          ...
          <div className="flex items-center gap-3 font-mono text-xs">
            ...
            <Link
              href="/terminal"
              className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-[#50C878] text-black font-bold text-xs hover:bg-[#00ff87] transition-all shadow-[0_0_15px_rgba(80,200,120,0.4)]"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={i} className="p-3 rounded-xl bg-[#0e131d]/90 border border-white/5 flex flex-col justify-between gap-1.5">
              <div className="flex items-center justify-between text-[10px] font-mono text-[var(--text-muted)]">
                <span>{m.label}</span>
                {m.icon}
              </div>
              <div className="flex items-baseline justify-between gap-1">
                <span className="text-sm sm:text-base font-mono font-bold text-white">{m.val}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          {/* Chart Preview Area */}
          <div className="lg:col-span-8 p-4 rounded-xl bg-[#0e131d]/90 border border-white/5 flex flex-col justify-between gap-4 h-64 sm:h-72 relative overflow-hidden">
            <div className="flex justify-between items-center text-xs font-mono">
              ...
              <div className="flex items-center gap-1 text-[10px]">
                <span className="px-2 py-0.5 rounded bg-[#50C878]/20 text-[#00ff87] font-bold">LIVE</span>
                <span className="px-2 py-0.5 text-[var(--text-muted)]">1D</span>
                <span className="px-2 py-0.5 text-[var(--text-muted)]">1W</span>
              </div>
            </div>
          ...
          <div className="lg:col-span-4 p-4 rounded-xl bg-[#0e131d]/90 border border-white/5 flex flex-col justify-between gap-3 h-64 sm:h-72">
            <div>
              ...
              <div className="flex flex-col gap-1 text-[11px] font-mono mt-2">
                {[
                  { bidV: '4,250', p: '2,980.25', askV: '1,450', bPct: '85%', aPct: '30%' },
                  { bidV: '6,100', p: '2,980.00', askV: '2,800', bPct: '95%', aPct: '45%' },
                  { bidV: '8,400', p: '2,979.75', askV: '3,900', bPct: '100%', aPct: '60%' },
                ].map((row, i) => (
                  <div key={i} className="flex justify-between items-center py-0.5 px-1 rounded bg-[#0a0d14]">
            ...
            <Link
              href="/terminal"
              className="w-full py-2 rounded-lg bg-white/5 hover:bg-white/10 text-center font-mono text-xs font-semibold text-[var(--text-primary)] hover:text-white transition-colors"
            >
