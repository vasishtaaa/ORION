  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#080b11] text-white w-full overflow-x-hidden">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-6">
        ...
        {/* Main Chat Interface */}
        <div className="w-full rounded-3xl bg-[#0e131d]/90 border border-white/10 p-0 flex flex-col h-[580px] sm:h-[650px] shadow-[0_12px_40px_rgba(0,0,0,0.7)] overflow-hidden">
          {/* Chat Header */}
          <div className="p-4 sm:p-5 border-b border-white/10 flex items-center justify-between bg-black/40">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#00ff87] to-[#006b3a] flex items-center justify-center text-black shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 flex flex-col gap-4 font-mono text-xs">
            {messages.map((m) => (
              ...
              >
                ...
                <div
                  className={`p-4 sm:p-5 rounded-2xl border relative group ${
                    m.sender === 'user'
            ...
            {isTyping && (
              <div className="flex items-center gap-2 text-[var(--matrix)] p-3.5 bg-[#0e131d] rounded-2xl border border-white/10 w-fit">
          ...
          <div className="px-4 sm:px-6 py-2.5 bg-black/40 border-t border-white/10 flex items-center gap-2 overflow-x-auto no-scrollbar">
            <span className="text-[10px] font-mono text-[var(--text-muted)] shrink-0">Presets:</span>
            {PROMPT_PRESETS.map((preset, i) => (
              <button
                key={i}
                onClick={() => handleSendMessage(preset)}
                className="w-fit px-3 py-1 rounded-full bg-[#0e131d] hover:bg-[#00ff87]/15 border border-white/10 text-[11px] font-mono text-[var(--text-secondary)] hover:text-[#00ff87] whitespace-nowrap transition-colors cursor-pointer"
              >
                {preset}
              </button>
            ))}
          ...
          <form
            ...
            className="p-4 sm:p-5 border-t border-white/10 bg-[#0a0d14] flex items-center gap-3"
          >
            <input
              type="text"
              value={inputText}
